...
from apps.users.permission.barman_permission import BarmanGroupPermission
...
class ItemsBarViewSet(viewsets.GenericViewSet):
    ...
    permission_classes = (IsAuthenticated,BarmanGroupPermission)
    ...
    def finishedStateItem(self,request):
        ...
        item.finished_state()
        item_serializer = self.serializer_class(item)
        Notification.objects.create(item=item, user=request.user)
        return Response(item_serializer.data,status=status.HTTP_200_OK)
